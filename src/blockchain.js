        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (timeElapsed < (5 * 60 * 1000)) {
                if (bitcoinMessage.verify(message, address, signature)) {
                    ...
                    const blockCreated = self._addBlock(block);
                    resolve(blockCreated);
                } else {
                    reject("Error: verification failed");
                }
            } else {
                reject("Error: sufficient time has not elapsed")
            }
    ...
    getBlockByHash(hash) {
        ...
        return new Promise((resolve, reject) => {
           ...
           if (result.length == 1) {
               ...
               reject("Error: chain in invalid state");
           }
        });
    }
        ...
        return new Promise((resolve, reject) => {
            ...
            self.chain.forEach(async (block) => {
                try {
                  ...
                  console.log(e);
                  reject(`Error: getStarsByWalletAddress - ${e}`);
                }
            });
            resolve(stars);
        });
        ...
        return new Promise(async (resolve, reject) => {
            for (let i = 2; i < self.chain.length; i++) {
                if (self.chain[i].previousBlockHash !== self.chain[i - 1].hash) {
                    errorLog.push(`Hash of Block at ${i} does not match with the previous hash`);
                }
            }
            resolve(errorLog);
        });
